let passwords = ["nyot babgr babgr kqtu kqtu kzshonp ylyk psqk";
...
let rec contains_duplicate cfn l l1 =
  match l with
  | [] -> false
  | h::t ->
    if cfn h l1 > 1 then
      ...
      contains_duplicate cfn t l1;;

let rec count_passwords cfn l =
  match l with
  | [] -> 0
  | h::t ->
    let list = String.split_on_char ' ' h in
      if contains_duplicate cfn list list then
        count_passwords cfn t
      else
        1 + count_passwords cfn t;;

print_int (count_passwords count passwords);;
...
let rec s_to_lc s =
  match s with
  | "" -> []
  | _ -> (String.get s 0) :: s_to_lc (String.sub s 1 ((String.length s) - 1));;

let s_to_rlc s = List.sort (fun a b -> if a = b then 0 else if a < b then -1 else 1) (s_to_lc s);;

let rec join l =
  match l with
  | [] -> ""
  | h::t -> Char.escaped h ^ join t;;

let rec convert l =
  match l with
  | [] -> []
  | h::t -> join (s_to_rlc h) :: convert t;;

let rec count_passwords2 cfn l =
  match l with
  | [] -> 0
  | h::t ->
    let list = convert (String.split_on_char ' ' h) in
      if contains_duplicate cfn list list then
        count_passwords2 cfn t
      else
        1 + count_passwords2 cfn t;;

print_int (count_passwords2 count passwords);;
