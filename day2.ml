...
let matrix = [[626;2424;2593;139;2136;163;1689;367;2235;125;2365;924;135;2583;1425;2502];
...
let rec checksum min max l =
  match l with
  | [] -> max - min
  | h::t ->
    let _min = if h < min then h else min in
    let _max = if h > max then h else max in
    checksum _min _max t;;

let rec process_matrix chs m =
  match m with
  | [] -> chs
  | h::t -> process_matrix (chs + (checksum max_int min_int h)) t;;

print_int (process_matrix 0 matrix);;
print_newline();;
...
let rec visit f l =
  ...
  | [] -> 0
  | h::t ->
    if f h then h else visit f t;;

let rec checksum2 chs l l1 =
  match l with
  | [] -> chs
  | h::t -> checksum2 (chs + (visit (fun a -> a != h && a mod h = 0) l1) / h) t l1;;

let rec checksum2_all chs l =
  match l with
  | [] -> chs
  | h::t -> checksum2_all (chs + (checksum2 0 h h)) t;;

print_int (checksum2_all 0 matrix);;
